...
# Do grid search over C:
pipeline = Pipeline([("features", full_pipeline), ("svm", svm)])
param_grid = dict(svm__C=[0.1, 1, 5])
...
# params['C'] = gsearch.best_params_['svm__C']
svm_final = SVC(kernel="poly",
          probability=True,
          C = gsearch.best_params_['svm__C'] )
...
df_score.to_csv('Data/Submission/submission_svm.csv', index=False)
